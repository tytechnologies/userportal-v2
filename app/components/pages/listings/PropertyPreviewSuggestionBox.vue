<template>
    <div class="dropdown relative inline-block text-left w-full cursor-pointer">
        <transition enter-active-class="transition ease-out duration-100"
            ...
            <div class="origin-top-right absolute right-0 w-full mt-1 rounded-md shadow-lg bg-card focus:outline-none z-10"
                ...
                <ul class="py-1" role="none" v-if="suggestions.length > 0">
                    <li v-for="(row, key) in suggestions" :key="key" class="p-2 hover:bg-muted">
                        <RowInfoCard :row="row" :showOnlineStatus="false" @click.native="$emit('input', row.id)" />
                    </li>
                </ul>
...
export default {
    ...
    components: { MenuDown, MenuUp, RowInfoCard },
    data() {
        return {
            show: false
        }
