<template>
  <div class="pt-6 px-11">
    <div class="flex mb-6 h-135">
      <div class="flex-1 mr-3 h-135">
        <div v-if="hasCurrentImage">
          <img
            class="object-cover w-full h-135"
    ...
    <div class="mb-8 text-lg font-medium text-center text-foreground pr-28">
...
export default {
  ...
  methods: {
    async fetchImages() {
      showLoading()
      try {
        const response = await axios.post('/api/listings/get-gallery-images', {
          listingId: this.listingId,
        })
        console.log('response from Gallery.vue: ', response)
        this.images = response.data.data
        console.log('images from Gallery.vue: ', this.images)
        this.setCurrentImage(0)
      } catch (error) {
        showToast({
          title:
            'Something went wrong fetching images. Please try again later.',
        })
      }
      dismissLoading()
    },
