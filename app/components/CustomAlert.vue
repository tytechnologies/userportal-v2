    ...
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
      ...
      >
        <div class="hi-modal inline-block align-bottom bg-card rounded-lg text-left overflow-hidden transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full" v-show="value">
          ...
          <div class="sm:px-4 py-3 sm:flex sm:flex-row-reverse">
            <button type="button" class="w-full inline-flex justify-center rounded-md border-transparent shadow-sm px-4 pt-1.5 pb-1 text-base font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-destructive sm:ml-3 sm:mr-auto sm:w-auto sm:text-sm"
              :class="computedConfirmButtonClass" @click="ok">
              {{ confirmButtonText }}
            </button>
            <button type="button" class="mt-3 w-full inline-flex justify-center rounded-md border-border shadow-sm px-4 pt-1.5 pb-1 text-base font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary sm:mt-0 sm:ml-auto sm:w-auto sm:text-sm"
