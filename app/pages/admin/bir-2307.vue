...
/**
 * /admin/bir-2307 — BIR Form 2307 (Creditable Tax Withheld at Source).
 *
 * Operator picks a date range, previews the per-vendor cert summary,
 * and saves the run for archival. Once saved, downloads CSV (the
 * cert package format) per period.
 *
 * Data source: completed work_orders with vendors that have a
 * non-zero withholding rate, aggregated per (vendor × ATC × period).
 */
...
useHead({ title: 'BIR Form 2307 | Admin' })
...
type Run = {
  ...
  period_start: string
  period_end: string
  totals: { gross_income_paid: number; tax_withheld: number; row_count: number }
  generated_at: string
  notes: string | null
}
...
const previewRows = ref<Form2307Row[]>([])
...
async function loadRuns() {
  ...
  try {
    const res = await $fetch<{ items: Run[] }>('/api/admin/bir-2307/runs')
...
async function preview() {
  ...
  try {
    const res = await $fetch<{ rows: Form2307Row[]; totals: any }>(
      '/api/admin/bir-2307/generate',
      {
        method: 'POST',
        body: {
...
async function saveRun() {
  ...
  saving.value = true
  try {
    await $fetch('/api/admin/bir-2307/generate', {
      method: 'POST',
      body: {
        period_start: periodStart.value,
        period_end: periodEnd.value,
    ...
    showToast({ title: 'Form 2307 run saved' })
    await loadRuns()
  } catch (err: any) {
...
function downloadCsv(runId: string) {
  window.location.href = `/api/admin/bir-2307/${runId}?format=csv`
}
    ...
    <template v-else-if="allowed">
      <header>
        <h1 class="text-page-title">
          BIR Form 2307 — Creditable Tax Withheld
        </h1>
        <p class="mt-1 text-sm text-muted-foreground">
          Aggregates withholding tax per (vendor × ATC code × period) from
          completed work orders. Save a run for archival; download CSV when
          you need the cert summary for printing.
        </p>
      </header>
      ...
      >
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-3">
          ...
          <div>
            <div class="text-[10px] font-medium uppercase tracking-wide text-muted-foreground">Tax withheld</div>
            <div class="mt-1 text-xl font-semibold tabular-nums text-success">
              {{ fmtPHP(previewTotals.tax_withheld) }}
            </div>
          </div>
          <div>
          ...
          <table class="min-w-full divide-y divide-border text-sm">
            <thead class="bg-muted/40">
              <tr>
                ...
                <th class="px-3 py-2 text-right text-xs font-medium uppercase tracking-wide text-muted-foreground">Withheld</th>
              </tr>
            </thead>
        ...
        <div v-else class="mt-4 text-sm text-muted-foreground">
          No vendor rows in this period. (Vendors without a withholding rate are excluded.)
        </div>
      </section>
          ...
          <table class="min-w-full divide-y divide-border text-sm">
            ...
            <tbody class="divide-y divide-border">
              <tr v-for="r in runs" :key="r.id">
                ...
                <td class="px-3 py-2 text-right tabular-nums text-success">
                  {{ fmtPHP(r.totals?.tax_withheld ?? 0) }}
                </td>
