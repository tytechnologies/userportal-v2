// Admin — list listing import batches with per-outcome counts.
//
// GET /api/admin/listings/import?page=&page_size=
//
// Same pattern as broker import list endpoint. Resolves uploader
// names + per-outcome row counts so the UI renders without extra
// round trips.
...
export default defineApiHandler({
  ...
  handler: async ({ event, query }) => {
    ...
    const { data: batches, error, count } = await (supabase as any)
      ...
      .select('id, source_label, source_id, uploaded_by, total_rows, processed_rows, status, created_at, processed_at',
              { count: 'exact' })
    ...
    return { batches: enriched, total: count ?? 0, page: q.page, page_size: q.page_size }
  },
})
