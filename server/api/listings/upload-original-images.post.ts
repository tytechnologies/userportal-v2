// Upload original-resolution (unwatermarked) images for a listing.
// Same hardening shape as upload-displayed-images — the only
// difference is the prefix (`properties/original/...`).
...
export default defineApiHandler({
  ...
  handler: async ({ event }) => {
    ...
    const prefix = `properties/original/property-${id}/`
      ...
      images.map(async (image: any, index: number) => {
        ...
        await client.send(
          new PutObjectCommand({
            Bucket: bucket,
            Key: `properties/original/${keyPath}`,
            Body: buffer,
